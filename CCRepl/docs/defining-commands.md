...
This document will describe the functionalities of `CommandBuilder` and provide examples at the end.
...
| Test | `.Test(std::function<bool(ReplContext&, CommandArgs&)> exec)` | Define test handler (see [defining-command-handlers.md](defining-command-handlers.md)). |
...
## Defining Names
A command only requires one parameter: the name. 
...
The full combination of command names is called the command **address**, and is automatically assigned.
...
Child `CommandBuilder` instances can be defined inside `CommandBuilder::Children()`.
...
- **CancelStrings:** A list of strings which can be entered to revert to default or cancel the command.
...
One addition is the subcommands `Name` and `Age` in `Data.Edit`, added to demonstrate `Mode()`.
	...
	Define(		
		...
		.Children(
			...
			.Children(
				...
				.Mode(2)
				.Desc("Edits item age.")
				.LongDesc("Edits an item age in the database. Uses (Y/N) confirmation before editing. Command behaviour can be altered by options:\n * '-f' ('force'): Bypasses (Y/N) confirmation.\n * '-q' ('quiet'): Edits without printing entry.")
